...
@WebMvcTest({WhiteListController.class, UserController.class})
class WhiteListControllerTest {
    ...
    void addToWhitelist_devuelve201() throws Exception {
        when(service.addToWhitelist(any())).thenReturn(new WhiteListDto.WishlistResponse(1L, 2L, 10L));
    ...
    void getByUser_devuelve200() throws Exception {
        when(service.getWhitelistByUser(2L)).thenReturn(List.of(new WhiteListDto.WishlistResponse(1L, 2L, 10L)));
    ...
    void check_devuelve200() throws Exception {
        when(service.isInWhitelist(2L, 10L)).thenReturn(true);
    ...
    void getUserById_devuelve200() throws Exception {
        when(service.getUserById(2L))
                .thenReturn(new WhiteListDto.UserDto(2L, "Ana", "ana@mail.com", "Calle 1"));
